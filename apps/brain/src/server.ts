...
dotenv.config(); // apps/brain/.env (if present)
...
const FEWSHOTS: { role: "user" | "assistant"; content: string }[] = [
  {
    role: "user",
    content:
      '{"text":"search wireless earbuds","context":{"url":"https://www.bestbuy.com","last_intents":[]}}',
  },
  {
    role: "assistant",
    content:
      '{"version":"1.0","intents":[{"type":"search","args":{"query":"wireless earbuds"},"priority":0,"requires_confirmation":false}],"context_updates":{"query":"wireless earbuds"},"confidence":0.9,"tts_summary":"Searching for wireless earbuds.","follow_up_question":null}',
  },
  {
    role: "user",
    content:
      '{"text":"open the second result","context":{"url":"https://www.bestbuy.com/site/searchpage.jsp?st=wireless+earbuds","last_intents":[{"type":"search","args":{"query":"wireless earbuds"}}]}}',
  },
  {
    role: "assistant",
    content:
      '{"version":"1.0","intents":[{"type":"click","args":{},"target":{"strategy":"auto","selector":"#search-list li:nth-of-type(2) a"},"priority":0,"requires_confirmation":false}],"context_updates":{},"confidence":0.8,"tts_summary":"Opening the second result.","follow_up_question":null}',
  },
  {
    role: "user",
    content:
      '{"text":"sort by price low to high","context":{"entity":"bestbuy"}}',
  },
  {
    role: "assistant",
    content:
      '{"version":"1.0","intents":[{"type":"sort","args":{"by":"price","order":"asc"},"priority":0,"requires_confirmation":false}],"context_updates":{},"confidence":0.85,"tts_summary":"Sorting by lowest price.","follow_up_question":null}',
  },
  // upload + submit example (note: content is a single JSON string; quotes are escaped correctly)
  {
    role: "user",
    content:
      '{"text":"upload my resume and submit the application","context":{"url":"https://careers.example.com/apply","last_intents":[{"type":"navigate","args":{"url":"https://careers.example.com/apply"}}]}}',
  },
  {
    role: "assistant",
    content:
      '{"version":"1.0","intents":[{"type":"upload","target":{"strategy":"auto","selector":"input[type=\\"file\\"]"},"args":{"fileRef":"resume://latest"},"priority":0,"requires_confirmation":true,"retries":1},{"type":"click","target":{"strategy":"text","text":"Submit"},"args":{},"priority":1,"requires_confirmation":true}],"context_updates":{},"confidence":0.75,"tts_summary":"I will upload your resume and then click submit. Please confirm.","follow_up_question":null}',
  },
  {
    role: "user",
    content:
      '{"text":"search wireless earbuds and extract the top 5 with prices","context":{"url":"https://www.bestbuy.com"}}',
  },
  {
    role: "assistant",
    content:
      '{"version":"1.0","intents":[{"type":"search","args":{"query":"wireless earbuds"},"priority":0,"requires_confirmation":false},{"type":"wait_for","args":{"selector":"[data-test=\\"results\\"]","timeoutMs":15000},"priority":1,"requires_confirmation":false},{"type":"extract_table","target":{"strategy":"auto","selector":"[data-test=\\"results\\"]"},"args":{"columns":["title","price"],"limit":5},"priority":2,"requires_confirmation":false}],"context_updates":{"query":"wireless earbuds"},"confidence":0.85,"tts_summary":"I’ll search and pull the top five items with prices.","follow_up_question":null}',
  },
];
...
export async function buildServer() {
  ...
  app.post("/parse", async (req, reply) => {
    ...
    if (!parsedReq.success) {
      return reply
        .status(400)
        .send({ error: "invalid_request", detail: parsedReq.error.format() });
    }
    ...
    try {
      ...
      if (!first.success) {
        json = await callLLMJSON([
          ...messages,
          {
            role: "system",
            content:
              "Your previous response failed schema validation. Return ONLY valid JSON matching the schema. No commentary.",
          },
        ]);
      }
    } catch (e: any) {
      return reply
        .status(500)
        .send({ error: "llm_error", detail: e?.message || String(e) });
    }
...
// --- robust "am I main?" check (works with tsx on Windows)
const isMain = (() => {
  try {
...
if (isMain && !process.env.VITEST) {
  ...
  buildServer()
    ...
    .catch((e) => {
      console.error("[brain-ts] failed to start:", e);
      process.exit(1);
    });
}
