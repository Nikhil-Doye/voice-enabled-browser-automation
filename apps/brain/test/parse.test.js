...
describe("brain-ts /parse", () => {
    ...
    it("handles upload with confirmation and tts", async () => {
        vi.spyOn(llm, "callLLMJSON").mockResolvedValue({
            ...
            context_updates: {},
            confidence: 0.75,
            tts_summary: "I will upload your resume and then click submit. Please confirm.",
        ...
        expect(res.statusCode).toBe(200);
        const j = res.json();
        expect(j.intents.some((i) => i.type === "upload")).toBe(true);
        expect(j.tts_summary).toBeTruthy();
        expect(j.intents.every((i) => typeof i.requires_confirmation === "boolean")).toBe(true);
    });
