...
export async function buildServer() {
  ...
  await app.register(cors, {
    origin: true, // or: ["http://127.0.0.1:5173", "http://localhost:5173"]
    credentials: false,
  });
  ...
  app.post("/uploads", async (req, reply) => {
    ...
    const res = await new Promise<{ fileRef: string; path: string }>(
      (resolve, reject) => {
        form.parse(req.raw as IncomingMessage, async (err, fields, files) => {
          if (err) return reject(err);
          const anyFile =
            Object.values(files)[0]?.[0] || Object.values(files)[0];
          if (!anyFile || !("filepath" in anyFile))
            return reject(new Error("no file"));
          const id = crypto.randomUUID();
          const ext =
            path.extname(anyFile.originalFilename || "") ||
  ...
  app.post("/close", async (req, reply) => {
    ...
    if (!id) return reply.status(400).send({ error: "session_id required" });
