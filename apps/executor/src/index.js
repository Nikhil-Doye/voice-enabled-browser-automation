...
import { describeIntent, validateNavigate, validateType, } from "./actions.js";
...
async function run() {
    const logs = [];
    const hasBB = Boolean(process.env.BROWSERBASE_API_KEY && process.env.BROWSERBASE_PROJECT_ID);
    if (!hasBB) {
        console.warn("[dry-run] No BROWSERBASE API vars set; running without real browser. Set them in your .env to test real sessions.");
